...
def rollout(env, exp_name, policies, species_indices, obs_filter, save_dict=True):
    ...
    """
    global iteration, agent_name_to_species_index_fn
    if type(env) == SexualColony:
        agent_name_to_species_index_fn = lambda name: 0
    ...
    while not done_dict['__all__']:
        ...
        for agent_name, raw_obs in raw_obs_dict.items():
            ...
            if type(obs_filter) is not dict:
                species_info[species_index]['obs'].append(obs_filter(raw_obs))
            else:
                species_info[species_index]['obs'].append(obs_filter[species_index](raw_obs))
        ...
        for species_index, info in species_info.items():
            ...
            if type(policies[species_index]) in (VDNMixer, VDNMixer_2):
